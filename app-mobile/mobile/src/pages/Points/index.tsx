...
const Points = () => {
    ...
    useEffect(() => {
        async function loadPosition(){
          ...
          if(status !==  'granted'){
            Alert.alert('Opss', 'Precisamos da permissão!!')
            return;
          }
    ...
    return (
        ...
        <View style={styles.itemsContainer}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:30}}>
                    {items.map(item =>(
                        <TouchableOpacity activeOpacity={0.6} key={String(item.id)} style={[styles.item, selectedItems.includes(item.id) ? styles.selectedItem : {}]}
